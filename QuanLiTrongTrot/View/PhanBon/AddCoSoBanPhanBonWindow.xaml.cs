...
{
    public partial class AddCoSoBanPhanBonWindow : Window
    {
        private int? _editId = null;
        private List<int> _selectedPhanBonIds = new List<int>();

        public AddCoSoBanPhanBonWindow()
        {
            InitializeComponent();
            LoadListBoxData();
        }

        public AddCoSoBanPhanBonWindow(DataRowView row) : this()
        {
            if (row != null)
            {
                ...
                this.Title = "Sửa Cơ Sở Bán Phân Bón";
            }
        }
        ...
        private void BtnXacNhan_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    // Thêm mới
                    int newId = DataProvider.GetNextId("CoSoBanPhanBon");
                    int newBanDoId = newId;
                    int firstLoaiId = _selectedPhanBonIds.First();

                    string queryInsert = @"
                        SET IDENTITY_INSERT CoSoBanPhanBon ON;
                        INSERT INTO CoSoBanPhanBon (Id, Ten, DiaChi, BanDoId, LoaiId) 
                        VALUES (@Id, @Ten, @DiaChi, @BanDoId, @LoaiId);
                        SET IDENTITY_INSERT CoSoBanPhanBon OFF;";
                ...
                else
                {
                    ...
                    string queryUpdate = @"UPDATE CoSoBanPhanBon 
        ...
        private void InsertPhanBonLinks(int coSoId)
        {
            foreach (int phanBonId in _selectedPhanBonIds)
            {
                try
                {
                    string query = "INSERT INTO PhanBon_CoSoBan (PhanBonId, CoSoBanId) VALUES (@PhanBonId, @CoSoId)";
        ...
        private void DeletePhanBonLinks(int coSoId)
        {
            try
            {
                string query = "DELETE FROM PhanBon_CoSoBan WHERE CoSoBanId = @CoSoId";
            ...
            else
            {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    try
                    {
                        ...
                        int deleteResult = DataProvider.DELETE_DATA(_editId.ToString(), "Id", "CoSoBanPhanBon");
                        if (deleteResult > 0)
                        {
