...
{
    /// <summary>
    /// Interaction logic for AddCoSoBanThuocWindow.xaml
    /// </summary>
    public partial class AddCoSoBanThuocWindow : Window
    {
        private int? _editId = null;
        private List<int> _selectedThuocIds = new List<int>();

        public AddCoSoBanThuocWindow()
        {
            InitializeComponent();
            LoadListBoxData();
        }

        public AddCoSoBanThuocWindow(DataRowView row) : this()
        {
            if (row != null)
            {
                ...
                this.Title = "Sửa Cơ Sở Bán Thuốc BVTV";
            }
        }
        ...
        private void BtnXacNhan_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                if (_editId == null)
                {
                    int newId = DataProvider.GetNextId("CoSoBanThuocBVTV");
                    int newBanDoId = newId;
                    int firstLoaiId = _selectedThuocIds.First();

                    string queryInsert = @"
                        SET IDENTITY_INSERT CoSoBanThuocBVTV ON;
                        INSERT INTO CoSoBanThuocBVTV (Id, Ten, DiaChi, BanDoId, LoaiId) 
                        VALUES (@Id, @Ten, @DiaChi, @BanDoId, @LoaiId);
                        SET IDENTITY_INSERT CoSoBanThuocBVTV OFF;";
                ...
                else
                {
                    ...
                    string queryUpdate = @"UPDATE CoSoBanThuocBVTV 
        ...
        private void InsertThuocLinks(int coSoId)
        {
            foreach (int thuocId in _selectedThuocIds)
            {
                try
                {
                    string query = "INSERT INTO ThuocBVTV_CoSoBan (ThuocBVTVId, CoSoBanId) VALUES (@ThuocId, @CoSoId)";
        ...
        private void DeleteThuocLinks(int coSoId)
        {
            try
            {
                string query = "DELETE FROM ThuocBVTV_CoSoBan WHERE CoSoBanId = @CoSoId";
        ...
        private void BtnXoa_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    ...
                    {
                        DeleteThuocLinks(_editId.Value);
                        int deleteResult = DataProvider.DELETE_DATA(_editId.ToString(), "Id", "CoSoBanThuocBVTV");
                        if (deleteResult > 0)
                        {
